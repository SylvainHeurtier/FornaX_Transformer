...
from constantes import LIM_FLUX_CLUSTER, LIM_FLUX_AGN, SEARCH_RADIUS_CLUSTER, SEARCH_RADIUS_AGN, WINDOW_SIZE_ARCMIN, NOMBRE_PHOTONS_MIN
...
if(Simulation2):
    ...
    catalog_path_aftXamin = os.path.expanduser('/lustre/fswork/projects/rech/wka/ufl73qn/TransformerProject/data/Simulation2/merged_catalog_cleaned.fits/merged_catalog_cleaned.fits')

if(Simulation3):
    name_dir = 'Simulation3'
    catalog_path_aftXamin = os.path.expanduser('/lustre/fswork/projects/rech/wka/ufl73qn/TransformerProject/data/Simulation3/merged_catalog_cleaned.fits')

catalog_path_AMAS  = os.path.expanduser(f'/results/{name_dir}/AMAS_matches_r{SEARCH_RADIUS_CLUSTER*3600:.0f}arcsec_flux{LIM_FLUX_CLUSTER}_40ks.fits')
catalog_path_AGN  = os.path.expanduser(f'/results/{name_dir}/AGN_matches_r{SEARCH_RADIUS_AGN*3600:.0f}arcsec_flux{LIM_FLUX_AGN}_40ks.fits')
...
# %%
SELECTED_COLUMNS_Xamin = ['EXT_LIKE', 'EXT', 'EXT_RA', 'EXT_DEC', 'EXT_RATE_MOS', 'EXT_RATE_PN', 
                          'PNT_DET_ML', 'PNT_RA', 'PNT_DEC', 'PNT_RATE_MOS', 'PNT_RATE_PN']
...
# %%
use_log_scale_Xamin = [True, True, False, False, True, True,
...
print(f"Jeu train: {len(data_AMAS_train)} >> {pourcentage_train:.1f}%")
print(f"Jeu test:  {len(data_AMAS_test)} >> {pourcentage_test:.1f}%")
...
print(f"Jeu train: {len(data_AGN_train)} >> {pourcentage_train:.1f}%")
print(f"Jeu test:  {len(data_AGN_test)} >> {pourcentage_test:.1f}%")
...
print(f"\nNombre total de sources Xamin: {len(data_Xamin)}")
print(f"Jeu train: {len(data_Xamin_train)} >> {len(data_Xamin_train) *100/len(data_Xamin):.1f}%")
print(f"Jeu test:  {len(data_Xamin_test)} >> {len(data_Xamin_test) *100/len(data_Xamin):.1f}%")
...
# %%
list_windows_test, info_clusters_test, info_AGN_test = Batisseuse2Fenetres(data_Xamin, data_AMAS_test, data_AGN_test, list_ID_Xamin_test)

# %% [markdown]
# Temps execution: 7m pour N>100ph

# %%
list_windows_train, info_clusters_train, info_AGN_train = Batisseuse2Fenetres(data_Xamin, data_AMAS_train, data_AGN_train, list_ID_Xamin_train)
...
# %% [markdown]
# Temps d'execution: 19m pour 2 rotations
...
print("\n=== AVANT AUGMENTATION ===")
print(f"Nombre de sources initiales: {len(list_windows_train)}")
...
print("\n=== APRÈS AUGMENTATION ===")
print(f"Nombre total de sources: {len(list_windows_train_augm)}")
...
# %% [markdown]
# Temps d'execution: **m pour 5 rotations
# 
# Temps d'execution: 3m pour 2 rotations
...
# Temps d'execution: **m pour 5 rotations
# 
# Temps d'execution: 28m pour 2 rotations
